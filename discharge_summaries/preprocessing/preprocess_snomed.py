"""Copied from https://github.com/CogStack/MedCAT/blob/master/medcat/utils/preprocess_snomed.py"""
...
def get_all_children(sctid, pt2ch):
    """
    Retrieves all the children of a given SNOMED CT ID (SCTID) from a given parent-to-child mapping
    (pt2ch) via the "IS A" relationship.
    ...
    Args:
        ...
        pt2ch (dict): A dictionary containing the parent-to-child relationships in the form
        {parent_sctid: [list of child sctids]}.
...
class Snomed:
    ...
    Attributes:
        ...
        uk_ext (bool, optional): Specifies whether the version is a SNOMED UK extension
        released after 2021. Defaults to False.
        uk_drug_ext (bool, optional): Specifies whether the version is a SNOMED UK drug
    ...
    def to_concept_df(self):
        ...
        Checks if the version is a UK extension release and sets the correct file names
        for the concept and description snapshots accordingly.
        Additionally, handles the divergent release format of the UK Drug Extension >v2021
        with the `uk_drug_ext` variable.
        ...
        for i, snomed_release in enumerate(snomed_releases):
            ...
            active_snomed_df["type_ids"] = active_snomed_df[
                "description_type_ids"
            ].apply(
                lambda x: int(hashlib.sha256(str(x).encode("utf-8")).hexdigest(), 16)
                % 10**8
            )
    ...
    def relationship2json(self, relationshipcode, output_jsonfile):
        ...
        for i, snomed_release in enumerate(snomed_releases):
            ...
            output_dict = {
                ...
                for key in set(list(output_dict.keys()) + list(relationship.keys()))
            }
    ...
    def map_snomed2icd10(self):
        """
        This function maps SNOMED CT concepts to ICD-10 codes using the refset mappings
        provided in the SNOMED CT release package.
    ...
    def map_snomed2opcs4(self):
        """
        This function maps SNOMED CT concepts to OPCS-4 codes using the refset mappings provided in the SNOMED CT
        release package.
    ...
    def _check_path_and_release(self):
        ...
        It looks for the "Snapshot" folder within the data path, and if it's not found, it looks for any folder
        containing the name "SnomedCT".
        It then stores the path and release in separate lists.
        If no valid paths are found, it raises a FileNotFoundError.

        Returns:
            tuple: a tuple containing two lists, the first one is a list of the paths where the data is located and the
            second is a list of the releases of the data.
    ...
    def _refset_df2dict(refset_df: pd.DataFrame) -> dict:
        ...
        The DataFrame should contain the columns 'referencedComponentId','mapTarget','mapGroup','mapPriority','mapRule',
        'mapAdvice'.
        ...
        """
        refset_dict = (
            refset_df.groupby("referencedComponentId")
            .apply(
                lambda group: [
                    {
                        "code": row["mapTarget"],
    ...
    def _map_snomed2refset(self):
        ...
        The refset mappings can either be ICD-10 codes in international releases or OPCS4 codes for SNOMED UK_extension,
        if available.

        Returns:
            pd.DataFrame: Dataframe containing SNOMED CT to refset mappings and metadata.
            OR
            tuple: Tuple of dataframes containing SNOMED CT to refset mappings and metadata (ICD-10, OPCS4),
            if uk_ext is True.
        ...
        for i, snomed_release in enumerate(snomed_releases):
            ...
            icd_mappings = mappings.sort_values(
                by=["referencedComponentId", "mapPriority", "mapGroup"]
